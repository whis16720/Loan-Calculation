...
document.querySelector("form").addEventListener("submit", (event) => {
    ...
    document.getElementById("originalAndInterest").value =
    ...
    `;
            ...
            const originalPerMonth =
                loan - Math.round(loan / months) * (months - 1);
            // Lãi hàng tháng
            const interestPerMonth =
                interest - Math.round((loan * rate) / 1200) * (months - 1);
            // Gốc và lãi hằng tháng
            const originalAndInterestPerMonth =
                originalPerMonth + interestPerMonth;
            html = `
        <td>${i}</td>
